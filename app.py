...
if "obligaciones_base" not in st.session_state:
  st.session_state.obligaciones_base = [
      ...
      {
          "id": "gas_m",
          "nombre": "Gas (Ya Separado)",
          "valor": 390000.0,
          "tipo": "Servicio Cuotas",
          "total": 12,
          "pagadas": 7,
          "periodo": "Mitad de Mes",
          "fecha_pago": "Día 15",
      ...
      {
          "id": "sist_mal",
          "nombre": "Sistecredito Maleta (Totto)",
          "valor": 66051.0,
          "tipo": "Crédito",
          "total": 4,
          "pagadas": 3,
      ...
      {
          "id": "tecno_moto",
          "nombre": "Tecnomecánica Moto",
          "valor": 223700.0,
          "tipo": "Crédito Moto",
          "total": 6,
          "pagadas": 0,
          "periodo": "Fin de Mes",
          "fecha_pago": "28 de Sept",
      },
      {
          "id": "gas_f",
          "nombre": "Gas (Fin)",
...
with col_cfg1:
  saldo_actual_banco = st.number_input(
      "Banco + Nequi (COP)", value=540427.0, step=10000.0
  )
with col_cfg2:
  nomina_quincenal_neta = st.number_input(
      "Nómina Neta", value=1294007.0, step=10000.0
  )
with col_cfg3:
...
with col_cfg4:
  quincena_tipo = st.selectbox(
      "Quincena",
      ["Mitad de Mes (Día 15)", "Fin de Mes (Día 20)"],
      key="select_quincena_main",
  )
st.markdown("</div>", unsafe_allow_html=True)

periodo_filtro = (
    "Mitad de Mes" if "Mitad" in quincena_tipo else "Fin de Mes"
)
...
total_pagado_obligaciones_actual = sum(
    ...
    and item["id"] != "gas_m"
    and pagos_actuales.get(item["id"], False)
)
...
total_deuda_global = sum(
    item["valor"]
    * max(
        0,
        (
            item["total"] - item["pagadas"]
            if "total" in item
            else (0 if "Reserva" in item["nombre"] else 1)
        ),
    )
    if "total" in item or "Reserva" not in item["nombre"]
    else item["valor"]
    for item in obligaciones_activas
)
...
with st.sidebar.expander("💾 Guardar / Cargar Respaldo"):
  ...
  st.download_button(
      "📥 Descargar Archivo Respaldo",
      data=json_str,
      file_name="respaldo_finanzas_tatis.json",
  ...
  if uploaded_file is not None:
    try:
      ...
      st.session_state.obligaciones_base = loaded_data.get(
          "obligaciones_base", st.session_state.obligaciones_base
      )
      st.session_state.pagos_por_periodo = loaded_data.get(
          "pagos_por_periodo", {}
      )
      st.session_state.imprevistos_por_periodo = loaded_data.get(
          "imprevistos_por_periodo", {}
      )
      st.session_state.prestamos_por_cobrar = loaded_data.get(
...
st.sidebar.markdown(
    "<hr style='border: 1px solid #BA68C8; margin: 10px 0;'>",
    unsafe_allow_html=True,
)
st.sidebar.markdown(
    "<h4 style='color: #4A148C; font-weight: 800; text-align: center; margin-"
    "bottom: 2px;'>💖 Distribución de Fondos</h4>",
    unsafe_allow_html=True,
)
...
val_pendiente = float(
    sum(
        ...
        for item in obligaciones_activas
        if item["periodo"] == periodo_filtro
        and not pagos_actuales.get(item["id"], False)
    )
)
...
df_grafico["Categoría"] = pd.Categorical(
    df_grafico["Categoría"], categories=orden_categorias, ordered=True
)
...
bar_chart = (
    ...
    .encode(
        ...
        y=alt.Y(
            "Categoría:N",
            title=None,
            axis=alt.Axis(labelFontSize=11, labelColor="#3E2723"),
        ),
        color=alt.Color(
            "Categoría:N",
            scale=alt.Scale(
...
items_filtrados = [
    item for item in obligaciones_activas if item["periodo"] == periodo_filtro
]
  ...
  for j in range(cols_por_fila):
    if i + j < len(items_filtrados):
      ...
      with cols[j]:
        ...
        info_sec = (
            f"Q {item['pagadas']}/{item['total']}"
            if "total" in item
            else f"{item.get('fecha_pago', '-')}"
        )
        estado_txt = "✅ Separado/Pagado" if esta_pagado else "⏳ Pendiente"
        ...
        if not esta_pagado:
          if st.button("Separar / Pagar ✅", key=f"pagar_{clave_periodo_actual}_{item_id}"):
...
st.markdown(
    "<hr style='border: 1px solid #CE93D8; margin: 15px 0;'>",
    unsafe_allow_html=True,
)

# PESTAÑAS INFERIORES PRO
tab_deudas, tab_resumen, tab_editar, tab_prestamos, tab_extras = st.tabs([
    "➕ Nueva Deuda",
    "📊 Resumen Quincena & Mes",
    "✏️ Modificar Cuotas",
    "🤝 Cuentas por Cobrar",
    "🚨 Imprevistos & Exportar",
])
...
with tab_deudas:
  with st.form(key="form_nueva_deuda_main"):
    ...
    with c_nd1:
      ...
      n_valor_cuota = st.number_input(
          "Valor Cuota / Deuda Total", min_value=0.0, step=10000.0
      )
    with c_nd2:
      n_total_cuotas = st.number_input(
          "Total Cuotas (1 si es pago único)", min_value=1, value=1, step=1
      )
      n_periodo = st.selectbox(
          "Asignar a Quincena / Tipo",
          [
              "Mitad de Mes",
              "Fin de Mes",
              "Deuda Libre / Sin Quincena Fija",
          ],
      )
    with c_nd3:
    ...
    if submit_deuda and n_nombre and n_valor_cuota > 0:
      ...
      st.session_state.obligaciones_base.append({
          ...
          "valor": n_valor_cuota,
          "tipo": (
              "Crédito Nuevo" if n_total_cuotas > 1 else "Deuda Única"
          ),
...
with tab_editar:
  st.markdown(
      "<p style='font-weight: 700; color: #4A148C;'>Si alguna cuota subió o"
      " bajó de precio este mes, selecciónala aquí y actualiza su valor:</p>",
      unsafe_allow_html=True,
  )
  with st.form(key="form_editar_cuota_val"):
    nombres_obligaciones = {
        item["nombre"] + f" ({item['periodo']})": item
        for item in st.session_state.obligaciones_base
    }
    sel_nombre_edit = st.selectbox(
        "Selecciona la obligación a modificar", list(nombres_obligaciones.keys())
    )
    item_a_editar = nombres_obligaciones[sel_nombre_edit]

    nuevo_valor_edit = st.number_input(
        "Nuevo Valor (COP)", value=float(item_a_editar["valor"]), step=1000.0
    )

    if st.form_submit_button("Actualizar Valor de Cuota 💜"):
      item_a_editar["valor"] = nuevo_valor_edit
      st.success(
          f"¡Se actualizó el valor de {item_a_editar['nombre']} a"
          f" {formato_COP(nuevo_valor_edit)}!"
      )
      st.rerun()
...
with tab_prestamos:
  with st.form(key="form_prestamo_main"):
    ...
    if (
        st.form_submit_button("Registrar Préstamo 🤝")
        and p_deudor
        and p_valor > 0
    ):
      st.session_state.prestamos_por_cobrar.append({
          "deudor": p_deudor,
          "valor": p_valor,
          "fecha_pago": p_fecha or "Por definir",
      })
  ...
  if st.session_state.prestamos_por_cobrar:
    ...
    for idx_p, prestamo in enumerate(
      ...
      st.markdown(
          f"**{prestamo['deudor']}** -"
          f" {formato_COP(prestamo['valor'])} (Paga:"
          f" {prestamo['fecha_pago']})"
      )
...
with tab_extras:
  ...
  with col_ex1:
    with st.form(key="form_imprevisto_main_central"):
      ...
      if (
          st.form_submit_button("Registrar Imprevisto")
          and nombre_imp_m
          and valor_imp_m > 0
      ):
        st.session_state.imprevistos_por_periodo[clave_periodo_actual].append({
            "nombre": nombre_imp_m,
            "valor": valor_imp_m,
        })
  ...
  with col_ex2:
    ...
    if st.button("📥 Descargar Base Completa en Excel (CSV)"):
      ...
      st.download_button(
          "Confirmar Descarga",
          data=csv_data,
          file_name="Finanzas_Tatis_Pro.csv",
          mime="text/csv",
      )
